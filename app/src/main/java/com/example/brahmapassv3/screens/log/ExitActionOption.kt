...
enum class ExitActionOption(val title: String) {
    EditExit("Edit exit"),
    ToggleFlag("Toggle flag"),
    DeleteExit("Delete exit");

    companion object {
        fun getByTitle(title: String): ExitActionOption {
        ...
        fun getOptions(hasEditOption: Boolean): List<String> {
            ...
            values().forEach { exitAction ->
                if (hasEditOption || exitAction != EditExit) {
                    options.add(exitAction.title)
                }
            }
            return options
        }
